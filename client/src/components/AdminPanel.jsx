...
export default function AdminPanel({ notification, onCreateRoom, roomId, customTasks, onAddTask, onDeleteTask, lastSaved }) {
  const [tab,              setTab]              = useState('game'); // 'game' | 'tasks'
  ...
  useEffect(() => {
    socket.emit('admin_get_custom_tasks');
  }, []);
  ...
  return (
    <div className="min-h-screen bg-cyber-dark cyber-grid flex flex-col items-center justify-center p-6 font-mono">
      ...
      <div className="w-full max-w-lg bg-cyber-panel border border-cyber-border">
        ...
        {/* Заголовок + табы */}
        <div className="border-b border-cyber-border px-8 pt-8 pb-0">
          <div className="text-cyber-purple text-lg font-bold tracking-widest mb-0.5">ПАНЕЛЬ УЧИТЕЛЯ</div>
          <div className="text-cyber-muted text-xs mb-4">Настройте параметры игровой сессии</div>
          <div className="flex gap-0">
            <button onClick={() => setTab('game')}
              className={`flex items-center gap-1.5 px-4 py-2 text-xs font-bold tracking-widest transition border-b-2
                ${tab === 'game' ? 'border-cyber-purple text-cyber-purple' : 'border-transparent text-cyber-muted hover:text-cyber-text'}`}>
              <Settings size={11} /> ИГРА
            </button>
